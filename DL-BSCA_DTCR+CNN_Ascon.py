...
for loss_type in ["CCE"]:
    ...
    _pred_cache_path = (trained_model_root +
                        f"predictions_{labeling_type}_{loss_type}_{dropout}_{model_idx}.npy")
    ...
    if os.path.exists(_pred_cache_path):
        ...
        if model_type == "mlp":
            jointed_predicted_hw, accuracy = run_mlp(X_train, X_test, combined_hws,
                                                    joint_hw_train,
                                                    joint_hw_test,
    ...
    NTGE = NTGE_fn(ge)
    print("NTGE:", NTGE)
    np.save(trained_model_root + f"{labeling_type}_{loss_type}_{dropout}_{model_idx}_v1.npy",
