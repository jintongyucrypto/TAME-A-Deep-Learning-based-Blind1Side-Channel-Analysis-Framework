...
parser.add_argument('--dropout', type=str, default=False, help='dropout')
parser.add_argument('--model_type', type=str, default='mlp', help='model_type')
parser.add_argument('--labeling_type', type=str, default='Mixture_DTCR', help='labeling_type')
...
labeling_total_acc, labeling_acc_m1, labeling_acc_m2, labeling_acc_y, labeling_acc_single_m1, labeling_acc_single_m2, labeling_acc_single_y = \
...
if val_source == 'gmm_center' and labeling_type == 'Mixture':
    ...
    if os.path.exists(_cluster_path) and os.path.exists(_means_path):
        ...
        _X_tmp = np.hstack((X_train[:, poi_m1], X_train[:, poi_m2], X_train[:, poi_y]))
    ...
    _X_joint      = np.hstack((X_train[:, poi_m1], X_train[:, poi_m2], X_train[:, poi_y]))
    ...
    if val_source == 'gmm_center':
        print(f"[WARNING] val_source='gmm_center' 仅支持 labeling_type='Mixture'，"
              f"当前为 '{labeling_type}'，已自动回退为 'random'。")
    _rng_val    = np.random.default_rng(val_seed)
...
val_label_acc, val_label_acc_m1, val_label_acc_m2, val_label_acc_y, val_label_acc_single_m1, val_label_acc_single_m2, val_label_acc_single_y = \
    customized_accuracy_ascon(val_assigned_labels, val_true_labels)

print(f"[Val 集标签质量]  总体: {val_label_acc:.4f}  "
      f"HW(m1): {val_label_acc_m1.mean():.4f} HW(m2): {val_label_acc_m2.mean():.4f} HW(y): {val_label_acc_y.mean():.4f}")
print(f"[对比整体训练集]  总体: {labeling_total_acc:.4f}  "
      f"HW(m1): {labeling_acc_m1.mean():.4f} HW(m2): {labeling_acc_m2.mean():.4f} HW(y): {labeling_acc_y.mean():.4f}")
...
np.save(_val_stats_path, {
    ...
    "val_label_acc_m1":     val_label_acc_m1,
    "val_label_acc_m2":     val_label_acc_m2,
    "val_label_acc_y":     val_label_acc_y,
    "overall_label_acc":   labeling_total_acc,
    "overall_label_acc_m1": labeling_acc_m1,
    "overall_label_acc_m2": labeling_acc_m2,
...
for loss_type in ["CCE"]:
    ...
    if os.path.exists(_pred_cache_path):
        ...
        if model_type == "mlp":
            jointed_predicted_hw, accuracy = run_mlp_lre(X_train, X_test, combined_hws,
                                                    X_val, y_val_clean,
                                                    joint_hw_train,
                                                    joint_hw_test,
            ...
            jointed_predicted_hw, accuracy = run_cnn_lre(X_train, X_test, combined_hws,
                                                    X_val, y_val_clean,
                                                    joint_hw_train,
                                                    joint_hw_test,
                                                    epochs, n_clusters, trained_model_root, base_models, model_idx,
    ...
    np.save(trained_model_root + f"{labeling_type}_{loss_type}_{nb_val}_{dropout}_{model_idx}.npy",
             ...
             "labeling_total_acc": labeling_total_acc, "labeling_acc_m1": labeling_acc_m1, "labeling_acc_m2": labeling_acc_m2,
